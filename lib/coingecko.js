...
module.exports = {
  getCryptoCurrencyMarketData: async (currency, crypto) => {
    ...
    for (crypto_currency of crypto) {
      ...
      const percentage_200d =
        data.data.market_data.price_change_percentage_200d;
  ...
  getCryptoCurrencyHistoricalData: async (currency, crypto) => {
    ...
    for (crypto_currency of crypto) {
      ...
      const all_time_high_percent =
        data.data.market_data.ath_change_percentage[currency];
      const all_time_high_date = data.data.market_data.ath_date[currency];
      const all_time_low = data.data.market_data.atl[currency];
      const all_time_low_percent =
        data.data.market_data.atl_change_percentage[currency];
      const all_time_low_date = data.data.market_data.atl_date[currency];
